...
@section('content')
    <div class="row">
        ...
        <div class="col-md-12 col-sm-12 col-md-offset-2 bienvenida">
            <h3>
                Participantes de la actividad {{$curso->nombre}} - Módulo {{$modulo->nombre}}<br/> Sección - {{$seccion}}
            </h3>
        </div>
            ...
            <div class="col-md-8 col-sm-8 opciones_part2">
                ...
                <div class="row">
                    <div class="col-md-6 col-md-offset-6">
                        {!!Form::open(["url"=>"profesor/actividades/".$curso->id."/modulos/".$modulo->id."/secciones/".$seccion."/participantes/buscar",  "method" => "GET" ])!!}
                            ...
                            @foreach($participantes as $index => $participante)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if(Entrust::can('ver_notas_profe'))
                                                {!!Form::open(["url"=>"profesor/actividades/".$curso->id."/modulos/".$modulo->id."/secciones/".$seccion."/participantes/".$participante->id."/notas",  "method" => "GET" ])!!}
                                ...
                                @else
                                    <tr>
                                        ...
                                        <td>
                                            @if(Entrust::can('ver_notas_profe'))
                                                {!!Form::open(["url"=>"profesor/actividades/".$curso->id."/modulos/".$modulo->id."/secciones/".$seccion."/participantes/".$participante[0]->id."/notas",  "method" => "GET" ])!!}
                                                <button type="submit" class="btn btn-info" data-toggle="tooltip" data-placement="bottom" title="Notas">
                                                    <span class="glyphicon glyphicon-book" aria-hidden="true"></span>
                                                </button>
                                                {!! Form::close() !!}
                ...
                @if(Entrust::can('ver_perfil_prof'))
                    <a href="{{URL::to("/")}}/profesor/actividades/{{$curso->id}}/modulos/{{$modulo->id}}/secciones" class="btn btn-default text-right"><span class="glyphicon glyphicon-remove"></span> Volver</a>
                @endif
            </div>
        @endif
    </div>
